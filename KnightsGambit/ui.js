...
let gameContainer, gameBoardWrapper, gameBoard, gridContent, uiPanel, levelDisplayElement,
    ...
    forestArmorAbilityButton, forestArmorContainer,
    // Profile UI Globals
    accountsOverlay, profilesListContainer, closeAccountsButton, newProfileOverlay,
    newProfileNameInput, confirmNewProfileButton, cancelNewProfileButton, currentProfileNameElement,
    accountsButton, newGameProfileButton;
...
function renderUnit(unit, parentElement = gridContent) {
    ...
    if (unit.isPolymorphed) {
        ...
        el.style.backgroundSize = "500% 200%"; // 5 columns, 2 rows
    } else {
        ...
        if (unit.team === 'player' && typeof equippedArmorId !== 'undefined' && equippedArmorId) {
            // Respect unit.armor_type if explicitly set (e.g. rescued prisoners forced to 'grey')
            if (unit.armor_type) {
    ...
    // Apply scale if defined in UNIT_DATA
...
function renderItem(item, parentElement = gridContent) {
    ...
    if (item.type === 'armor' || item.type === 'helmet' || item.type === 'flame_cloak' || item.type === 'flame_ring' || item.type === 'war_bow') {
        el.classList.add('special-item-glow');
        // Spawn particles after a short delay to ensure positioning is correct
        setTimeout(() => createGoldenGlowParticles(item.x, item.y, item.id), 50);
    }
...
function updateUnitGearOverlays(unit) {
    ...
    if (typeof playerAbilityUpgrades !== 'undefined' && playerAbilityUpgrades.war_bow > 0 && unit.type === 'archer') {
...
function updateTurnDisplay() { if (!endTurnButton) return; const isPlayer = currentTurn === 'player'; let actionsText = '', buttonText = `<span class="hotkey-text">E</span>nd Turn`, buttonTitle = "End Player Turn [E]"; let isButtonDisabled = false, hasDisabledClass = false, isNextLevelMode = false; if ((typeof isLevelCleared === 'function' ? isLevelCleared() : levelClearedAwaitingInput)) { actionsText = 'Level Cleared!'; buttonText = `Proc<span class="hotkey-text">e</span>ed`; buttonTitle = "Proceed [E]"; isNextLevelMode = true; isButtonDisabled = false; hasDisabledClass = false; } else if (isPlayer) { const remainingActions = units.reduce((count, unit) => count + (unit.team === 'player' && !unit.acted && !unit.isFrozen && !unit.isNetted && isUnitAliveAndValid(unit) ? (unit.canQuickStrike && unit.actionsTakenThisTurn < 1 ? 2 : 1) - unit.actionsTakenThisTurn : 0), 0); actionsText = `Actions Left: ${remainingActions}`; isButtonDisabled = isProcessing; hasDisabledClass = isProcessing; } else { actionsText = `Enemy Turn...`; buttonTitle = "Enemy Turn"; isButtonDisabled = true; hasDisabledClass = true; } if (actionsLeftDisplayElement) actionsLeftDisplayElement.textContent = actionsText; endTurnButton.innerHTML = buttonText; endTurnButton.title = buttonTitle; endTurnButton.disabled = isButtonDisabled; endTurnButton.classList.toggle('disabled', hasDisabledClass); endTurnButton.classList.toggle('next-level-mode', isNextLevelMode); }
...
function updateUnitInfo(unit) {
    ...
    if (show) {
        ...
        if (unit.isPolymorphed) {
            ...
            unitPortraitElement.style.backgroundSize = "500% 200%"; // 5 columns, 2 rows
        } else {
...
async function handleUnitDeathAnimation(unit, deathX, deathY, timeoutMap) {
    return new Promise((resolve) => {
        ...
        // Immediately clear ALL selection-related classes and styling
        el.classList.remove('selected', 'selected-enemy', 'acted', 'frozen', 'netted', 'slowed', 'stealthed', 'bleeding', 'elite', 'in-tower');
        el.classList.add('dead');
        el.style.pointerEvents = 'none';
        ...
        const fadeTimeoutId = setTimeout(() => {
            ...
            const removeTimeoutId = setTimeout(() => {
                el.remove();
                // HP bar removed immediately now
                timeoutMap.delete(unit.id + '-remove');
                resolve();
...
async function handleCellClick(event) {
    ...
    // For Flame Wave, allow clicking anywhere on any row - skip obstacle checks
...
async function handleUnitClick(event, clickedUnit) {
    ...
    if (currentTurn === 'player') {
        if (selectedUnit && selectedUnit.team === 'player') {
            ...
            // Deselect any currently selected enemy first
            if (selectedUnit && selectedUnit.team === 'enemy') {
                deselectUnit(false, false); // Don't play sound for silent switch
            }
...
async function handleObstacleClick(event, clickedObstacle) {
    ...
    // Flame Wave can target anywhere on a row - NOTHING blocks it
    if (currentSpell === 'flameWave' && clickedObstacle) {
        await castSpell('flameWave', { x: clickedObstacle.x, y: clickedObstacle.y });
        return;
    }
...
function handleKeyDown(event) {
    ...
    if (event.key.toLowerCase() === 'f' && event.shiftKey) { toggleFullscreen(); event.preventDefault(); return; }
    if (event.key.toLowerCase() === 'z') { if (toggleHpBarsSetting) toggleHpBarsSetting.checked = !toggleHpBarsSetting.checked; toggleWorldHpBarsVisibility(); event.preventDefault(); return; }
    ...
    if (event.shiftKey && gameRunning) {
        ...
        if (key === 'b') {
            event.preventDefault(); unlimitedSpellsCheat = !unlimitedSpellsCheat; if (unlimitedSpellsCheat) {
            ...
            event.preventDefault();
            // Allow skipping even if processing (enemy turn), force it.
            // if (isProcessing) return; // REMOVED check to allow skip
...
function isAnyOverlayVisible(excludeMainMenu = false) { return isGameOverScreenVisible() || isMenuOpen() || isLeaderboardOpen() || isLevelSelectOpen() || isShopOpen() || isLevelCompleteOpen() || isChooseTroopsScreenOpen() || isSettingsOpen() || isAchievementsOpen() || isNamePromptOpen() || (!excludeMainMenu && isMainMenuOpen()); }
function hideAllOverlays() { const overlays = [mainMenu, gameOverScreen, menuOverlay, leaderboardOverlay, levelSelectScreen, shopScreen, levelCompleteScreen, chooseTroopsScreen, settingsOverlay, achievementsOverlay, namePromptOverlay]; overlays.forEach(o => { o?.classList.add('hidden'); o?.classList.remove('visible'); }); gameBoardWrapper?.classList.toggle('active', isGameActive() && !isAnyOverlayVisible()); startTooltipUpdater(); }
...
function saveScoreToLeaderboard(level, gold, name, achievements = 0) {
    try {
        const leaderboard = JSON.parse(localStorage.getItem(LEADERBOARD_STORAGE_KEY) || '[]');
        const safeName = typeof name === 'string' ? name.substring(0, 12).trim() : "Hero";

        const existingIndex = leaderboard.findIndex(item => item.name === safeName);
        if (existingIndex !== -1) {
            // Update only if the new level is higher
            if (level > leaderboard[existingIndex].level) {
                leaderboard[existingIndex].level = level;
                leaderboard[existingIndex].gold = gold; // Still store gold as secondary metric
                leaderboard[existingIndex].achievements = achievements;
                leaderboard[existingIndex].date = new Date().toISOString().split('T')[0];
            } else if (level === leaderboard[existingIndex].level && achievements > (leaderboard[existingIndex].achievements || 0)) {
                // Same level but more achievements
                leaderboard[existingIndex].achievements = achievements;
                leaderboard[existingIndex].gold = Math.max(gold, leaderboard[existingIndex].gold);
            }
        } else {
            leaderboard.push({ name: safeName, level, gold, achievements, date: new Date().toISOString().split('T')[0] });
        }

        leaderboard.sort((a, b) => {
            if (b.level !== a.level) return b.level - a.level;
            if (b.achievements !== a.achievements) return (b.achievements || 0) - (a.achievements || 0);
            return b.gold - a.gold;
        });

        localStorage.setItem(LEADERBOARD_STORAGE_KEY, JSON.stringify(leaderboard.slice(0, MAX_LEADERBOARD_ENTRIES)));
    } catch (e) {
...
function generateArmoryCards() {
    ...
    unitsToGenerate.forEach(unitType => {
        ...
        header.innerHTML = `<div class="shop-icon-container" style="width: 4em; height: 4em; margin-bottom: 0;"><span class="shop-item-icon icon icon-unit-${unitType}" title="${unitName}"></span></div> <h4>${unitName}</h4>`;
...
function getShopItemTitleColor(itemType, itemId, ownedLevel = 0) {
    ...
    if (itemType === 'armor' || itemType === 'helmet') {
        ...
        // Green for uncommon (level 2)
        if (ownedLevel >= 2) return 'var(--color-green-bright)';
...
function updateShopDisplay() {
    ...
    if (tabArmory) {
        // Unlock condition: Must BEAT level 60 (so highestLevelReached > 60)
        ...
        if (!isArmoryUnlocked) {
            ...
            // Ensure we are not on the locked tab
    ...
    shopItemsContainer.querySelectorAll('.shop-item').forEach(item => {
        ...
        if (itemType === 'recruit') {
            ...
            // Locked if not rescued
            if (unitType === 'champion' && !unlockedUnits.champion) isLocked = true;
            if (unitType === 'rogue' && !unlockedUnits.rogue) isLocked = true;
            if (unitType === 'archer' && !unlockedUnits.archer) isLocked = true;
            if (unitType === 'wizard' && !unlockedUnits.wizard) isLocked = true;
            ...
            cost = PASSIVE_UPGRADE_COSTS.tactical_command; const currentBonusSlots = playerPassiveUpgrades.tactical_command || 0; const currentRosterSize = MAX_ACTIVE_ROSTER_SIZE_BASE + currentBonusSlots; isLocked = totalOwnedUnits < TACTICAL_COMMAND_UNLOCK_UNITS; isMaxed = currentRosterSize >= MAX_ACTIVE_ROSTER_SIZE_MAX; canBuy = playerGold >= cost && !isLocked && !isMaxed; item.dataset.currentCost = cost; if (costSpan) costSpan.textContent = cost;
            // Update title to show roster size prominently
            if (titleElement) {
            ...
            // Explicitly add locked class if level 0 so it looks greyed out
            ...
            if (iconImg && armorId && ARMOR_DATA[armorId]) {
                ...
                if (data.iconPosition) {
                    ...
                    // Unit portraits use: backgroundSize = "500% 200%" (5 cols, 2 rows).
            ...
            if (playerOwnedArmor.flame_cloak) {
                ...
                canBuy = false;
                if (titleElement) titleElement.style.color = 'var(--color-orange-highlight)';
        ...
        if (costLine && itemType !== 'passive') { // Exclude passive (Gold Magnet) which uses "Drop Only"
            if (isMaxed) {
                if (itemType === 'ability_upgrade' || itemType === 'passive_purchase') {
                    costLine.innerHTML = `<span style="color: var(--color-text-muted); font-weight: bold;">Purchased</span>`;
                } else {
                ...
                if (!costLine.querySelector('.shop-item-cost')) {
                    costLine.innerHTML = `Cost: <span class="shop-item-cost">${cost}</span><span class="icon icon-inline icon-gold" title="Gold" aria-label="Gold"></span>`;
...
function handleConfirmTroops() {
    ...
    if (totalActive === 0) {
        playSfx('error');
        chooseTroopsFeedback.textContent = "Roster cannot be empty.";
        chooseTroopsFeedback.className = 'shop-message error';
        setTimeout(() => {
            if (chooseTroopsFeedback) {
                chooseTroopsFeedback.textContent = '';
                chooseTroopsFeedback.className = 'shop-message';
            }
        }, 2000);
        return;
    }
    if (totalActive > maxActiveRosterSize) {
        playSfx('error');
        chooseTroopsFeedback.textContent = `Roster max ${maxActiveRosterSize}.`;
        chooseTroopsFeedback.className = 'shop-message error';
        setTimeout(() => {
            if (chooseTroopsFeedback) {
                chooseTroopsFeedback.textContent = '';
    ...
    const levelToStart = levelToStartAfterManage;
    troopScreenOrigin = '';
    levelToStartAfterManage = 0;

    if (origin === 'shop') {
        showShop(currentShopOrigin, shopIsBetweenLevels);
    } else if (origin === 'restart' || (levelToStart > 0 && origin !== 'levelSelect')) {
        initGame(levelToStart);
    } else {
        showLevelSelect();
    }
    startTooltipUpdater();
...
function handleMuteToggle(updateSettingFlag = true) {
    isMuted = !isMuted;
    bgMusic.muted = isMuted;
    victoryMusicPlayer.muted = isMuted;
    Object.values(sfx).forEach(sound => { if (sound) sound.muted = isMuted; });
...
function getTooltipTarget(el) {
    ...
    if (isShopOpen()) {
        ...
        if (shopItemEl) {
            ...
            // Check data-type for generic shop-items
    ...
    if (!tooltipElement || data === null || typeof data === 'undefined') { hideTooltip(); return; } let content = ''; try {
        switch (type) {
            case 'unit':
                ...
                break; case 'item': const item = data; const itemConfig = ITEM_DATA[item.type]; if (!itemConfig) break; if (item.type === 'gold') content = `<span style="color:var(--color-gold-light); font-weight:bold;">Gold Coin</span><br><span style="color:#ffffff;">Value: ${itemConfig.value || 1}</span>`; else if (item.type === 'chest') { content = `<span style="color:var(--color-gold-light); font-weight:bold;">Chest</span>`; if (item.opened) content += `<br><span style="color:#ffffff;">Empty</span>`; } else if (item.type === 'health_potion') { const potionCycle = Math.floor((currentLevel - 1) / TOTAL_LEVELS_BASE); const healAmount = (HEALTH_POTION_HEAL_AMOUNT || 1) + potionCycle; content = `<span style="color:#ff4444; font-weight:bold;">Health Potion</span><br><span style="color:#ffffff;">Restores +${healAmount} HP</span>`; } else if (item.type === 'shiny_gem') content = `<span style="color:var(--color-gold-light); font-weight:bold;">Shiny Gem</span><br><span style="color:#ffffff;">Value: ${item.value || '?'}</span>`; else if (item.type === 'gold_magnet') content = `<span style="color:var(--color-gold-light); font-weight:bold;">Gold Magnet</span><br><span style="color:#ffffff;">Pulls nearby gold!</span>`; else if (item.type === 'spellbook') content = `<span style="color:var(--color-gold-light); font-weight:bold;">Spellbook</span><br><span style="color:#ffffff;">Restores 1 spell charge.</span>`; else if (item.type === 'armor' || item.type === 'helmet' || item.type === 'flame_cloak') {
                    ...
                    if (armorData) {
                        ...
                        if (['green', 'blue', 'red'].includes(armorId)) {
                        ...
                        if (armorId === 'flame_cloak' || item.type === 'flame_cloak') color = 'var(--color-orange-highlight)';
                            ...
                            if (ownedLevel >= 5) color = 'var(--color-purple-bright)'; // Purple
                            else if (ownedLevel >= 3) color = 'var(--color-blue-bright)'; // Blue
                            else color = 'var(--color-green-bright)'; // Green
                        }
                ...
                } else if (item.type === 'flame_ring' || item.type === 'war_bow') {
                    const itemConf = ITEM_DATA[item.type];
                    const itemColor = itemConf.color || '#ffffff';
                    content = `<span style="color:${itemColor}; font-weight:bold;">${itemConf.name}</span><br><span style="color:#ffffff; font-size: 0.85em; opacity: 0.9;">${itemConf.description}</span>`;
                }
                break;
            case 'obstacle': const obstacle = data; const obsConfig = OBSTACLE_DATA[obstacle.type];
                ...
                if (obstacle.destructible && !hideHpFor.includes(obstacle.type)) content += `<br>HP: ${obstacle.hp}/${obstacle.maxHp}`; if (obstacle.enterable) { const occupant = obstacle.occupantUnitId ? units.find(u => u.id === obstacle.occupantUnitId && isUnitAliveAndValid(u)) : null; content += `<br>${occupant ? `Occupied by ${occupant.name}` : 'Empty'}`; if (occupant?.baseRange > 1) content += ` (+${obstacle.rangeBonus} RNG)`; if (!occupant && obstacle.hp > 0) content += `<br><span style="color:#cccccc;">(Enter/Exit from below)</span>`; } if (obsConfig.blocksLOS) content += `<br><span style="color:#ffccaa;">Blocks Line of Sight</span>`; if (obstacle.hidesUnit && !obstacle.revealed) content += `<br><span style="color:#aadeff;">Seems suspicious...</span>`; if (obstacle.canBeAttacked) content += `<br><span style="color:#ff4444;">Attackable</span>`; break; case 'shop': const shopItemId = data.dataset.itemId; const shopItemType = data.dataset.type; if (shopItemType === 'recruit') {
                    ...
                    if (unitType === 'champion' && !unlockedUnits.champion) content += `<br><span style="color:#ff4444;">Rescue from Lvl 24.</span>`;
                    if (unitType === 'rogue' && !unlockedUnits.rogue) content += `<br><span style="color:#ff4444;">Rescue from Lvl 40.</span>`;
                    if (unitType === 'archer' && !unlockedUnits.archer) content += `<br><span style="color:#ff4444;">Rescue from Lvl 10.</span>`;
                    if (unitType === 'wizard' && !unlockedUnits.wizard) content += `<br><span style="color:#ff4444;">Rescue from Lvl ${WIZARD_UNLOCK_LEVEL}.</span>`;
                    ...
                    if (highestLevelReached < unlockLevel) {
                        content += `<br><span style="color:#ff4444;">Requires ${UNIT_DATA[unitForUpgrade]?.name || 'Unit'} Unlocked.</span>`;
                    }
                    ...
                    if (highestLevelReached < requiredLevel) content += `<br><span style="color:#ff4444;">Requires Lvl ${requiredLevel}.</span>`;
                    ...
                    if (highestLevelReached < unlockLevel && unitType !== 'knight') {
                        content += `<br><span style="color:#ff4444;">Requires ${UNIT_DATA[unitType]?.name || 'Unit'} Unlocked.</span>`;
                    }
                    ...
                    if (highestLevelReached < requiredLevel) content += `<br><span style="color:#ff4444;">Requires Level ${requiredLevel}.</span>`;
                    ...
                    if (highestLevelReached <= reqLvl && currentLevel === 0) content += `<br><span style="color:#ff4444;">Requires completing Level ${reqLvl}.</span>`;
            ...
            case 'passive':
                const passiveId = data.dataset.itemId?.startsWith('passive_') ? data.dataset.itemId.substring(8) : (data.dataset.itemId === 'tactical_command' ? 'tactical_command' : null);
                if (passiveId && PASSIVE_DATA[passiveId]) {
                    ...
                    content = `<span style="color:var(--color-gold-light); font-weight:bold;">${passiveConfig.name}${passiveId === 'gold_magnet' ? ` (Lvl ${passiveLevel})` : ''}</span>`;
                    content += `<br><span style="color:#ffffff;">${passiveConfig.description}</span>`;
                    if (passiveId === 'tactical_command') {
                        const tcCost = PASSIVE_UPGRADE_COSTS.tactical_command;
                        const currentSlots = MAX_ACTIVE_ROSTER_SIZE_BASE + (playerPassiveUpgrades.tactical_command || 0);
                        const canBuyMore = currentSlots < MAX_ACTIVE_ROSTER_SIZE_MAX;
                        if (canBuyMore) {
                            content += `<br><span style="color:#ffffff;">(Currently: ${currentSlots} slots)</span>`;
                            const reqUnits = parseInt(data.dataset.requiredUnits);
                            const ownedUnits = Object.values(playerOwnedUnits).reduce((s, c) => s + c, 0);
                            if (ownedUnits < reqUnits) content += `<br><span style="color:#ff4444;">Requires owning ${reqUnits}+ total units.</span>`;
                        } else content += `<br><span style="color:#ffaa00;">(Max Slots Reached)</span>`;
                    }
                } break;
            case 'armor':
                const armorId = data.dataset.armorId;
            ...
            case 'bonus':
                ...
                else if (bonusType === 'fullHp') { bTitle = "Flawless Victory"; bDesc = "Complete level with full HP."; }
                else if (bonusType === 'noLosses') { bTitle = "No Man Left Behind"; bDesc = "Complete level without losing any units."; }
                else if (bonusType === 'noArmor') { bTitle = "Born Leader"; bDesc = "Complete level with No Armor equipped (no losses)."; }

                content = `<span style="color:var(--color-gold-light); font-weight:bold;">${bTitle}</span><br><span style="color:#ffffff;">${bDesc}</span>`;
                break;
            case 'armory_tab':
                const isArmoryUnlocked = highestLevelReached > 60;
                content = `<span style="color:var(--color-gold-light); font-weight:bold;">Armory</span>`;
                content += `<br><span style="color:#ffffff;">Upgrade units and special skills.</span>`;
                if (!isArmoryUnlocked) {
                    content += `<br><span style="color:#ff4444;">Requires beating Level 60.</span>`;
                }
                break;
            case 'debug_unit':
                const uData = data;
...
function createExplosionEffect(x, y, type, splashRadius = 0) {
    ...
    if (type === 'fireball') {
        ...
        // Target -8 widths because steps(8) divides total distance into 8 jumps.
        // We want 8 discrete frames: 0, -1, ..., -7.
        // Moving to -8 with steps(8) covers this range correctly.
        explosion.animate([
...
function populateDebugEnemyGrid() {
    ...
    enemyTypes.forEach(enemyType => {
        ...
        if (unitData) {
            ...
            } else if (unitData.forceCssVariant === 'purple') {
                // From getUnitFilterString line 520 & 555 (mapped from V'tharak logic if variantType was purple, but here it's forceCssVariant)
                // V'tharak config has forceCssVariant: 'purple'. 
                // getUnitFilterString checks unit.variantType === 'purple'. 
                // gameLogic.js sets unit.variantType = data.forceCssVariant.
                // So yes, it should be purple.
...
function updateForestArmorButton() {
    ...
    const isForestArmorEquipped = equippedArmorId === 'green';
...
async function showLeaderboardLists() {
    ...
    globalEntries.forEach(entry => {
        mergedMap.set(entry.name, { ...entry, source: 'global' });
    });

    // Merge local
    localEntries.forEach(entry => {
        const existing = mergedMap.get(entry.name);
        if (!existing || entry.level > existing.level || (entry.level === existing.level && (entry.achievements || 0) > (existing.achievements || 0))) {
            mergedMap.set(entry.name, { ...entry, source: 'local' });
        }
    });

    let combinedEntries = Array.from(mergedMap.values());
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    startGameButton?.addEventListener('click', () => {
        playSfx('select');
        if (!gameSettings.playerName || gameSettings.playerName === "Hero") {
            showNamePrompt();
        } else {
            hideMainMenu();
            showLevelSelect();
    ...
    shopExitButton?.addEventListener('click', () => { playSfx('menuClose'); hideShop(); proceedAfterShopMaybe(); });
    shopTroopsButton?.addEventListener('click', () => { if (!isShopOpen()) return; playSfx('select'); hideShop(); showChooseTroopsScreen(shopIsBetweenLevels ? 0 : currentLevel, 'shop'); });
    ...
    if (acceptNakedChallengeButton) {
        acceptNakedChallengeButton.addEventListener('click', () => {
            ...
            updateShopDisplay();
            if (shopFeedback) {
                shopFeedback.textContent = "Naked Challenge accepted! No Armor locked.";
                shopFeedback.className = 'shop-message success';
            }
        });
    }
    ...
    if (declineNakedChallengeButton) {
        declineNakedChallengeButton.addEventListener('click', () => {
            ...
            updateShopDisplay();
            if (shopFeedback) {
                shopFeedback.textContent = "Equipped No Armor.";
                shopFeedback.className = 'shop-message success';
            }
        });
    }
    ...
    profiles.forEach(name => {
        ...
        item.innerHTML = `
            <div class="p-info">
                ...
                <div class="p-status">Level: ${level} | ${achCount}/26 Achievements</div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key.toLowerCase() === 't' && !e.shiftKey && isGameActiveFlag && !isAnyOverlayVisible()) {
            ...
            units.forEach(u => {
                if (u.team === 'player' && isUnitAliveAndValid(u) && u.element) {
