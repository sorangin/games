...
function renderItem(item, parentElement = gridContent) {
    ...
    if (item.type === 'armor' || item.type === 'helmet' || item.type === 'flame_cloak' || item.type === 'flame_ring' || item.type === 'war_bow' || item.type === 'tome_of_chain_lightning') {
        el.classList.add('special-item-glow');
        // Spawn particles after a short delay to ensure positioning is correct
        setTimeout(() => {
            if (item.type === 'tome_of_chain_lightning') {
...
function handleKeyDown(event) {
    ...
    if (isShopOpen() && event.key.length === 1 && !event.ctrlKey && !event.altKey && !event.metaKey) {
        const key = event.key.toLowerCase();
        if (key === 'e') { shopExitButton?.click(); event.preventDefault(); return; }
        if (key === 's') {
            const tab = document.getElementById('shop-tab-main');
            if (tab) { playSfx('select'); currentShopTab = 'main'; updateShopDisplay(); }
        ...
        if (key === 'a') {
            ...
            const tab = document.getElementById('shop-tab-armory');
            if (tab) { playSfx('select'); currentShopTab = 'armory'; updateShopDisplay(); }
            event.preventDefault(); return;
        }
        if (key === 't') {
            const tab = document.getElementById('shop-tab-troops');
            if (tab) { playSfx('select'); currentShopTab = 'troops'; updateShopDisplay(); }
            event.preventDefault(); return;
        }
    }
...
function updateLevelSelectPagination() {
    ...
    // Always show buttons, just disable them if not applicable
    levelSelectPrevPage.disabled = currentLevelSelectPage <= 1;
    levelSelectNextPage.disabled = currentLevelSelectPage >= totalPages;

    // Ensure they are visible (removing any lingering classes)
    levelSelectPrevPage.classList.remove('hidden', 'invisible');
    levelSelectNextPage.classList.remove('hidden', 'invisible');

    if (totalPages <= 1) {
        // Keeping them visible but disabled is safer to prevent layout shifts
    } else {
        if (levelSelectPageInfo) levelSelectPageInfo.classList.remove('hidden');
    }
}
...
function updateShopDisplay() {
    ...
    const hideTooltipOnScroll = () => { if (typeof hideTooltip === 'function') hideTooltip(); };
    if (shopItemsContainer) {
        shopItemsContainer.addEventListener('scroll', hideTooltipOnScroll, { passive: true });
        shopItemsContainer.addEventListener('touchmove', hideTooltipOnScroll, { passive: true });
    }
    if (scrollContainer) {
    ...
    shopItemsContainer.querySelectorAll('.shop-item').forEach(item => {
        ...
        if (itemType === 'recruit') {
            ...
            // Hide cost for locked/unrescued units
            ...
            if (pId === 'tactical_command') {
                ...
                const lootedFirst = (playerPassiveUpgrades.gold_magnet || 0) > 0;
                if (!lootedFirst) {
                ...
                isLocked = false;
                isMaxed = true; // For now
                if (costSpan) costSpan.textContent = 'Drop Only';
            } else {
                if (passiveLevelSpan) passiveLevelSpan.textContent = ''; // Hide level for one-time passives
                isMaxed = (playerPassiveUpgrades[pId] || 0) >= 1;
                isLocked = false;
                if (pId === 'thorns' || pId === 'vampiric_aura' || pId === 'evasion') {
                    const reqLvl = parseInt(item.dataset.requiredLevel) || (pId === 'thorns' ? 35 : (pId === 'vampiric_aura' ? 50 : 25));
                    isLocked = highestLevelReached < reqLvl;
                    if (isLocked) {
                        // Requirement info in tooltip only
                    }
                }
            }
            canBuy = playerGold >= cost && !isLocked && !isMaxed;
...
function updateChooseTroopsScreen() {
    ...
    allPlayerUnitTypes.forEach(unitType => {
        ...
        const iconStyle = `background-image: ${styles.backgroundImage}; background-position: ${styles.backgroundPosition}; background-size: ${styles.backgroundSize}; background-repeat: no-repeat; display: inline-block; vertical-align: middle;`;

        if (active > 0) {
            const card = document.createElement('div');
            card.className = 'troop-card';
            card.dataset.unitType = unitType;
            card.innerHTML = `<div style="${iconStyle}" title="${unitData.name}"></div><span class="troop-count">${active}</span>`;
        ...
        if (available > 0) {
            ...
            card.className = 'troop-card';
            card.dataset.unitType = unitType;
            card.innerHTML = `<div style="${iconStyle}" title="${unitData.name}"></div><span class="troop-count">${available}</span>`;
    ...
    try {
        switch (type) {
            case 'unit':
                ...
                } else if (item.type === 'tome_of_chain_lightning') {
                    content = `<span style="color:cyan; font-weight:bold;">Tome of Chain Lightning</span><br><span style="color:#ffffff; font-size: 0.85em; opacity: 0.9;">Reduces Chain Lightning cooldown by 1 turn.</span>`;
                }
                break;
            case 'obstacle': const obstacle = data; const obsConfig = OBSTACLE_DATA[obstacle.type];
                ...
                if (obstacle.destructible && !hideHpFor.includes(obstacle.type)) content += `<br>HP: ${obstacle.hp}/${obstacle.maxHp}`; if (obstacle.enterable) { const occupant = obstacle.occupantUnitId ? units.find(u => u.id === obstacle.occupantUnitId && isUnitAliveAndValid(u)) : null; content += `<br>${occupant ? `Occupied by ${occupant.name}` : 'Empty'}`; if (occupant?.baseRange > 1) content += ` (+${obstacle.rangeBonus} RNG)`; if (!occupant && obstacle.hp > 0) content += `<br><span style="color:#cccccc;">(Enter/Exit from below)</span>`; } if (obsConfig.blocksLOS) content += `<br><span style="color:#ffccaa;">Blocks Line of Sight</span>`; if (obstacle.hidesUnit && !obstacle.revealed) content += `<br><span style="color:#aadeff;">Seems suspicious...</span>`; if (obstacle.canBeAttacked) content += `<br><span style="color:#ff4444;">Attackable</span>`; break; case 'shop': const shopItemId = data.dataset.itemId; const shopItemType = data.dataset.type; if (shopItemType === 'recruit') {
                    ...
                    // Removed "Rescue from Lvl X" hints - keep it a surprise
                    ...
                    if (highestLevelReached <= reqLvl && currentLevel === 0) content += `<br><span style="color:#ff4444;">Requires Level ${reqLvl}.</span>`;
            ...
            case 'passive':
                ...
                if (passiveId && PASSIVE_DATA[passiveId]) {
                    ...
                    // For one-time passives (Loot Hoarder, Evasion, Thorns, Vampiric Aura), don't show level in tooltip title
                    const showLevelInTooltip = passiveLevel > 0 && passiveId !== 'loot_hoarder' && passiveId !== 'evasion' && passiveId !== 'thorns' && passiveId !== 'vampiric_aura';
                    ...
                    if (passiveId === 'tactical_command') {
                        ...
                        // General Passives Level Requirements
                        if (passiveConfig.requiredLevel) {
                            const reqLvl = passiveConfig.requiredLevel;
                            const color = highestLevelReached >= reqLvl ? '#ffffff' : '#ff4444';
                            content += `<br><span style="color:${color};">Requires Level ${reqLvl}</span>`;
                        }
                    }

                    const cost = PASSIVE_UPGRADE_COSTS[passiveId] || 0;
                    if (cost > 0 && passiveLevel === 0) content += `<br><span style="color:#ffddaa;">Cost: ${cost} Gold</span>`;
                } break;
            case 'armor':
                const armorId = data.dataset.armorId;
                const armorData = ARMOR_DATA[armorId];
...
function updateForestArmorButton() {
    ...
    if (!forestArmorContainer) return;
    const isForestArmorEquipped = typeof window.equippedArmorId !== 'undefined' ? window.equippedArmorId === 'green' : false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    startGameButton?.addEventListener('click', () => {
        playSfx('select');
        let currentName = (window.gameSettings && window.gameSettings.playerName) ? window.gameSettings.playerName : null;
        ...
        if (!currentName || currentName === "Hero") {
            try {
                    ...
                    if (profiles && profiles.length > 0) {
                        ...
                        if (lastName) {
                            ...
                            if (typeof loadGameData === 'function') {
                                ...
                                if (window.gameSettings) window.gameSettings.playerName = currentName;
                ...
                if (currentName && currentName !== "Hero" && window.gameSettings) {
                    window.gameSettings.playerName = currentName;
                }
