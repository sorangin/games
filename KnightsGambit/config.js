...
function playMusic(trackUrl) {
    ...
    } else if (audioInitialized && bgMusic.paused) {
...
function playDefeatMusic() {
    if (isMuted || !audioInitialized || typeof Audio === 'undefined' || !DEFEAT_MUSIC) return;
    stopMusic();
...
function selectAndLoadMusic() {
    ...
    if (isBattle) {
        if (typeof units !== 'undefined' && Array.isArray(units)) {
            const hasBoss = units.some(u => u.type === 'orc_juggernaut');
            if (hasBoss) {
...
function getUnitSpriteUrl(type, variant = 'grey', armorType = 'grey') {
    ...
    // Fallback for missing 'none' sprites to base 'grey'
