...
function spawnEnemies() {
    ...
    window.parTurns = Math.floor(((currentGridCols + currentGridRows) / 4) + enemyCount + (bossCount * 3));
...
async function attack(attacker, targetX, targetY) {
    ...
    try {
        ...
        if (effectiveDamage > 0 && (currentTargetObject.team === 'player' || currentUnitInTower?.team === 'player') && playerPassiveUpgrades.thorns > 0) {
...
function resetCheatStatus() {
    isPlayerCheater = false;
    unlimitedSpellsCheat = false;
    playerCheatSpellAttackBonus = 0;
    saveGameData();
    console.log("Cheat status has been reset for this profile.");
    if (typeof updateCheatResetButtonVisibility === 'function') updateCheatResetButtonVisibility();
    if (typeof showFeedback === 'function') showFeedback("Cheat status reset!", "feedback-success");
}

window.resetCheatStatus = resetCheatStatus;
...
function saveScoreToLeaderboard(level, gold, name, achievements = 0) {
    ...
    const armorId = (typeof equippedArmorId !== 'undefined') ? equippedArmorId : 'grey';
    const helmetId = (typeof equippedHelmetId !== 'undefined') ? equippedHelmetId : 'none';
    const flameCloak = (typeof equippedFlameCloak !== 'undefined') ? equippedFlameCloak : false;
    ...
    lb.push({
        ...
        nakedChallenge: (typeof isNakedChallengeActive !== 'undefined') ? isNakedChallengeActive : false,
    ...
    // Also attempt online submission if possible
    if (window.OnlineLeaderboard && window.OnlineLeaderboard.isInitialized()) {
        const isCheater = (typeof isPlayerCheater !== 'undefined' && isPlayerCheater);
