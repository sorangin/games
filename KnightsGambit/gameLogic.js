...
let playerAbilityUpgrades = { rogue_quickstrike: 0, war_bow: 0, flame_ring: 0 };
...
async function preloadAssetsAndStart() {
    ...
    // Apply mute state to audio objects if muted in settings
...
function initGame(startLevel = 1) {
    ...
    try {
        ...
        // playSfx('startBeep'); // Removed to fix Error sound issue
...
function getFilteredEnemyPool(effectivePool, introUnitsThisLevel) {


    return {


        common: (effectivePool.common || []).filter(type => {


            if (introUnitsThisLevel.includes(type)) return false;


            if (type === 'goblin_archer') return currentLevel >= GOBLIN_ARCHER_INTRO_LEVEL;


            if (type === 'goblin_sapper') return currentLevel >= GOBLIN_SAPPER_INTRO_LEVEL;
        ...
        uncommon: (effectivePool.uncommon || []).filter(type => {
            ...
            if (type === 'goblin_club') return currentLevel >= CLUBBER_INTRO_LEVEL;


            if (type === 'goblin_netter') return currentLevel >= GOBLIN_NETTER_INTRO_LEVEL;


            if (type === 'goblin_shaman') return currentLevel >= GOBLIN_SHAMAN_INTRO_LEVEL;


            if (type === 'goblin_sapper') return currentLevel >= GOBLIN_SAPPER_INTRO_LEVEL;


            if (type === 'goblin_pyromancer') return currentLevel >= GOBLIN_PYROMANCER_INTRO_LEVEL;
            if (type === 'goblin_shadowstalker') return currentLevel >= GOBLIN_SHADOWSTALKER_INTRO_LEVEL;
            return true;
        ...
        rare: (effectivePool.rare || []).filter(type => {
...
function getEnemyTypesToSpawn(numEnemies, effectivePool, introUnitsThisLevel) {
    ...
    Object.keys(introLevels).forEach(type => {
        if (currentLevel === introLevels[type]) {
            // Rare units (V'tharak, Zulkash, Zulfar) do NOT spawn automatically on their intro level.
            // They follow their small random chance from the beginning.
            const isRare = (type === 'vtharak' || type === 'zulkash' || type === 'zulfar');
    ...
    const filteredPool = getFilteredEnemyPool(effectivePool, introUnitsThisLevel);
    ...
    if (remainingCount > 0) {
        ...
        (filteredPool.rare || []).forEach(type => {
            if (type !== 'zulkash' && type !== 'zulfar' && type !== 'vtharak') {
...
function createUnit(type, x, y, variantType = 'green', isElite = false, infiniteHpBonus = 0, infiniteAtkBonus = 0) {
    ...
    const unit = {
        ...
        ... ((({
            ...
            canStealth, canQuickStrike, isBoss, dropsArmor, dropsWarBow, dropsFlameCloak, dropsFlameRing, dropsGoblinMotherSkull, isTotem, healAmount,
            ...
            isBoss: isBoss ?? false, dropsArmor: dropsArmor ?? false, dropsWarBow: dropsWarBow ?? false, dropsFlameCloak: dropsFlameCloak ?? false, dropsFlameRing: dropsFlameRing ?? false, dropsGoblinMotherSkull: dropsGoblinMotherSkull ?? false,
    ...
    if (unit.team === 'player') {
        ...
        if (equippedWarBow && unit.baseRange > 1) {
            ...
            switch (itemData.pickupAction) {
                ...
                case 'unlockWarBow':
                    ...
                    if (!playerAbilityUpgrades.war_bow) {
                        ...
                        playerAbilityUpgrades.war_bow = 1;
                        equippedWarBow = true; // Auto-equip when looted
                        ...
                        if (typeof showFeedback === 'function') {
                            ...
                            setTimeout(() => showFeedback(`War Bow Unlocked! (+1 Range for Archers)`, 'feedback-levelup'), feedbackDelay);
                        ...
                        // Apply range bonus immediately to current units


                        units.forEach(u => {


                            if (u.team === 'player' && u.baseRange > 1) {
                                ...
                                if (u.inTower) {
                                    ...
                                    const tower = obstacles.find(o => o.id === u.inTower);


                                    if (tower) u.currentRange += tower.rangeBonus;
                ...
                case 'unlockFlameRing':
                    ...
                    if (!playerAbilityUpgrades.flame_ring) {
                        ...
                        equippedFlameRing = true; // Auto-equip when looted
...
async function attack(attacker, targetX, targetY) {
    ...
    try {
        ...
        if ((currentTargetObject.team === 'player' || currentUnitInTower?.team === 'player') && typeof playerPassiveUpgrades !== 'undefined' && playerPassiveUpgrades.evasion > 0) {
            if (Math.random() < 0.05) {
                if (typeof showFeedback === 'function') showFeedback("Dodged!", "feedback-success");
        ...
        // Apply Fire Resistance/Immunity for Fire Attacks (including Wizard/Pyromancer regular attacks)
        if (effectiveProjectileType === 'fireball' || attacker.type === 'goblin_pyromancer' || attacker.type === 'zulfar') {
            ...
            if (currentTargetObject.team === 'player') {
                if (typeof equippedArmorId !== 'undefined' && ARMOR_DATA[equippedArmorId]) {
                    if (equippedArmorId === 'red') {
                        totalFireResistance += Math.floor((playerOwnedArmor['red'] || 1) / 2);
                    }
        ...
        // Thorns: Melee attackers take 1 damage (only if attacker is adjacent)
        if ((currentTargetObject.team === 'player' || currentUnitInTower?.team === 'player') && playerPassiveUpgrades.thorns > 0) {
            ...
            if (dist <= 1 && attacker.team === 'enemy') {
                attacker.hp -= 1;
                if (typeof showDamagePopup === 'function') showDamagePopup(attacker.x, attacker.y, 1);
        ...
        if (attacker.fireballRadius > 0 && attacker.shootsProjectileType === 'fireball' && isRanged) {
            ...
            if (typeof createExplosionEffect === 'function') {
                ...
                createExplosionEffect(targetCenterX, targetCenterY, 'fireball', splashRadius);
    ...
    return new Promise(resolve => {
        ...
        if ((obsType === 'barrel' || obsType === 'crate') && obstacle.dropsLoot) {
            ...
            if (Math.random() <= dropChance) {
                ...
                if (Math.random() < GEM_DROP_CHANCE_BARREL) {
        ...
        if (obsType === 'exploding_barrel' && obstacle.explodes) {
            ...
            playSfx('fireballHit'); // Or add a dedicated explosion sound
...
async function removeUnit(unit) {
    ...
    if (unitTeam === 'enemy' && !unit.isTotem) {
        ...
        if (isBoss && dropsArmorFlag) {
        ...
        else if (dropsGoblinMotherSkullFlag) {
            ...
            if (gemDropCell) itemsToDrop.push(createItem('shiny_gem', gemDropCell.x, gemDropCell.y, 0));
                ...
                if (unit.totemCooldown <= 0) {
                    ...
                    const alliesInRange = getUnitsInArea(unit.x, unit.y, SHAMAN_TOTEM_RANGE)
                        ...
                        .filter(u => {
                            ...
                            // Only heal units within Manhattan distance of 5 tiles


                            const distance = Math.abs(u.x - unit.x) + Math.abs(u.y - unit.y);


                            return u.team === 'enemy' && !u.isTotem && u.hp < u.maxHp && isUnitAliveAndValid(u) && distance <= SHAMAN_TOTEM_RANGE;


                        });


                    // Heal ALL units in range, not just one
                    ...
                    if (validSpawns.length > 0) {
                        ...
                        for (let i = 0; i < count && validSpawns.length > 0; i++) {
                            ...
                            if (newGoblin) {
                                ...
                                if (gameSettings.showHpBars && typeof createWorldHpBar === 'function') createWorldHpBar(newGoblin);
...
function applyFlameWaveDamage(targetRow, damage, targetTeam) {
    ...
    for (let x = 0; x < currentGridCols; x++) {
        ...
        if (fw_unit && fw_unit.team === targetTeam && isUnitAliveAndValid(fw_unit)) {
            ...
            if (!fw_unit.inTower) {
                ...
                if (fw_unit.isInvulnerableForTurn) {
                    ...
                    playSfx('hit'); fw_unit.hp -= actualDamageFW;
                }
...
async function performAIAction(unit) {
    ...
    const livingPlayers = units.filter(u => u.team === 'player' && isUnitAliveAndValid(u) && (!u.isStealthed || getDistance(unit, u) <= 1));
    ...
    if (unit.flees) {
        ...
        if (!actionTaken) {
            ...
            const targetPlayer = livingPlayers.reduce((closest, player) => {
        ...
        if (!actionTaken && unit.suicideExplode) {
            ...
            if (minDist <= unit.explosionRadius) { await explodeUnit(unit); return; }
                ...
                if (path && path.length > 0) {
                    // Use as much of the path as the unit's MOV allows (1-indexed mov, 0-indexed path)
                    const maxMoveIndex = Math.min(unit.mov - 1, path.length - 1);
                    const targetCell = path[maxMoveIndex];
                    // Verify this cell is in valid moves
                    if (getValidMoves(unit).some(m => m.x === targetCell.x && m.y === targetCell.y)) {
                        await moveUnit(unit, targetCell.x, targetCell.y); hasMoved = true;
        ...
        else if (!actionTaken && (unit.type === 'goblin_pyromancer' || unit.type === 'zulkash')) {
                ...
                if (bestRow !== null) {
                    ...
                    for (let i = 0; i < numRows; i++) {
                        ...
                        if (targetRow >= 0 && targetRow < currentGridRows) {
                            ...
                            pendingFlameWaves.push({ row: targetRow, damage: unit.atk });
...
async function castChainLightning(caster, targetUnit) {
    ...
    // Find ALL connected enemies using flood-fill algorithm (BFS)


    // This finds every enemy that connects through adjacent tiles (including diagonals)
    ...
    const chainTargetsOrdered = []; // Stores { unit, sourceUnit } in order found
    ...
    while (chainQueue.length > 0) {
        ...
        const adjacentCells = getAdjacentCells(current.x, current.y, true); // true = include diagonals
        ...
        for (const cell of adjacentCells) {
            ...
            if (unitAtCell &&
                ...
                unitAtCell.team === 'enemy' &&


                !visitedIds.has(unitAtCell.id) &&


                isUnitAliveAndValid(unitAtCell)) {
                ...
                // Add this unit's position to queue to check its neighbors
    ...
    for (let i = 0; i < chainTargetsOrdered.length; i++) {
        ...
        // Visual delay for chain effect (staggered)


        setTimeout(async () => {


            if (isUnitAliveAndValid(chainTarget)) {


                // Chain lightning visual from source to this target
    ...
    // chainTargetsOrdered contains the jumped targets. primary target is separate.
    const totalHits = 1 + chainTargetsOrdered.length; // 1 (primary) + jumps
...
function castPolymorph(caster, targetUnit) {
    ...
    // Transform to Sheep - KEEP CURRENT HP (fix: don't heal to full sheep HP)
    ...
    // Do NOT change maxHp or hp - keep original values
...
function startNextLevel() { if (isGameOver()) return; if (victoryMusicPlayer) { victoryMusicPlayer.pause(); victoryMusicPlayer.currentTime = 0; } currentLevel++; levelToRestartOnLoss = currentLevel; levelClearedAwaitingInput = false; initGame(currentLevel); }
...
function purchaseUnit(unitType) {
    ...
    if (unitType === 'champion' && highestLevelReached < 24) {


        playSfx('error');


        showFeedback("Requires Level 24!", "feedback-error");
    ...
    if (unitType === 'rogue' && highestLevelReached < 40) {


        playSfx('error');


        showFeedback("Requires Level 40!", "feedback-error");
    ...
    if (unitType === 'archer' && highestLevelReached < 10) {


        playSfx('error');


        showFeedback("Requires Level 10!", "feedback-error");
...
function purchasePassive(passiveId) {
    ...
    if (playerGold >= cost) {
        playerGold -= cost;
        playerPassiveUpgrades[passiveId] = currentLevel + 1;
        saveGameData();
        return true;
    }
    return false;
...
function switchProfile(name) {
    ...
    saveGameData(); // Initial save for new profile (creates keys for auto-discovery)
    if (typeof updateUiForNewLevel === 'function') updateUiForNewLevel();
...
function loadGameData() {
    try {
        ...
        const defaultAbilityUpgrades = { rogue_quickstrike: 0, war_bow: 0, flame_ring: 0 };
        ...
        if (key === 'playerName' && typeof value === 'string') {
            ...
            if (gameSettings.showHpBars !== boolValue) {
                ...
                if (typeof updateWorldHpBarsVisibility === 'function') {
                    // Only update if game has started to avoid "random healthbar" on menu
                    if (gameStarted) updateWorldHpBarsVisibility();
                }
        ...
        try {
            ...
            switch (condition.type) {
                ...
                case 'level_complete_condition': if (eventType === 'level_complete') {
                    ...
                    if (condition.condition === 'lone_wolf') {
                        const playerUnitCount = units.filter(u => u.team === 'player' && isUnitAliveAndValid(u)).length;
                        if (playerUnitCount === 1) conditionMet = true;
                    }
                } break;
...
function completeLevelAndShowSummary() {
    ...
    if (window.rescuedUnitsThisLevel && window.rescuedUnitsThisLevel.length > 0) {
        window.rescuedUnitsThisLevel.forEach(unitType => {
            if (!unlockedUnits[unitType]) {
                ...
                // Even if unlocked, maybe give them another one? 
                // Original logic gave one on first unlock. User request implies just "unlocking". 
                // We'll stick to unlocking. 
            }
        });
        saveGameData();
    ...
    if (typeof saveScoreToLeaderboard === 'function') {
        ...
        if (window.OnlineLeaderboard && window.OnlineLeaderboard.isInitialized()) {
            ...
            const isCheater = (typeof unlimitedSpellsCheat !== 'undefined' && unlimitedSpellsCheat) || (typeof playerCheatSpellAttackBonus !== 'undefined' && playerCheatSpellAttackBonus > 0);


            window.OnlineLeaderboard.submitScore(gameSettings.playerName, currentLevel, achCount, isCheater, isNakedChallengeActive);
...
function debugSpawnEnemy(enemyType, x, y, variant = 'green') {
    ...
    // Create HP bar if enabled


    if (gameSettings.showHpBars && typeof createWorldHpBar === 'function') {
...
/**
 * Saves a new score to the local leaderboard.
 */
function saveScoreToLeaderboard(level, gold, name, achievements = 0) {
    ...
    localStorage.setItem('knightGambitLeaderboard', JSON.stringify(top20));
}
window.saveScoreToLeaderboard = saveScoreToLeaderboard;

// Ensure core variables used by ui.js are global
