...
function activateRogueStealth(unit) {
    ...
    if (unit.isStealthed) {
        ...
        playSfx('rogueStealth'); // Or a different sound for unstealth
...
function activateRogueQuickStrike(unit) {
    ...
    if (unit.quickStrikeUsedThisLevel) {
        ...
        showFeedback("Quick Strike already used this level.", "feedback-error");
            ...
            switch (itemData.pickupAction) {
                ...
                case 'unlockWarBow':
                    ...
                    if (typeof showFeedback === 'function') {
                        ...
                        setTimeout(() => showFeedback("War Bow unlocked! Ranged units +1 RNG!", 'feedback-levelup'), feedbackDelay);
                ...
                case 'unlockFlameCloak':
                    ...
                    if (typeof showFeedback === 'function') {
                        ...
                        setTimeout(() => showFeedback("Flame Cloak obtained! +2 HP, Fire Immune!", 'feedback-levelup'), feedbackDelay);
...
async function explodeUnit(unit, isDeathExplosion = false) {
    ...
    affectedUnits.forEach(targetUnit => {
        ...
        if (targetUnit.team === 'player') {
            // Strict User Request: No Player Friendly Fire from Explosions
...
function removeObstacle(obstacle) {
    ...
    return new Promise(resolve => {
        ...
        if (obstacle.occupantUnitId) {
            ...
            if (unitInside) leaveTower(unitInside);
                ...
                if (fbTargetObject && originElement && targetPos) {
                    ...
                    if (stillTarget) {
                        ...
                        if (tookDamage) {
                            ...
                            // Unstealth if rogue is hit by Fireball


                            if (stillTarget.type === 'rogue' && stillTarget.isStealthed) {
    ...
    for (let x = 0; x < currentGridCols; x++) {
        ...
        if (fw_unit && fw_unit.team === targetTeam && isUnitAliveAndValid(fw_unit)) {
            ...
            if (!fw_unit.inTower) {
                ...
                if (fw_unit.team === 'player' || fw_unit.isInvulnerableForTurn) {
                    // Strict Immunity
                } else {
            ...
            if (attackTargetsFB.units.includes(finalTargetObject.id) && hasLineOfSight(unit, finalTargetObject)) {
                ...
                if (unit.atk > 0) {
                    ...
                    if (stillTarget) {
                        ...
                        // Unstealth if rogue is hit by Pyromancer Fireball
...
async function castChainLightning(caster, targetUnit) {
    ...
    caster.chainLightningCooldown = unlimitedSpellsCheat ? 0 : UNIT_DATA.wizard.chainLightningCooldown;
...
function deleteProfile(name) {
    ...
    if (gameSettings.playerName === name) {
        const nextHero = updatedProfiles[0] || "Hero";
...
function completeLevelAndShowSummary() {
    ...
    if (highestLevelReached <= 60 && currentLevel === 60) {
        ...
        if (typeof showFeedback === 'function') setTimeout(() => showFeedback("Armory Unlocked! Visit Shop.", "feedback-achievement-unlock", 4000), 1000);
